...
**v1 修复**：`projects.resolve` 失败时回退 adapter 实时 `listWorkspaces()`
（动态反映 DSH workspaceRegistry），匹配即补注册——新增/删除工作区即时生效。

**v2（本版，2026-08-26 用户授权"开放所有工作区权限"）**：实时列表也匹配
不到时，**直接把传入路径注册并激活**——`code_use_workspace` 接受任意绝对
路径，不再要求预先注册。敏感目录可读风险由授权人承担。

**应用**：
```bash
node patches/workspace-refresh.mjs
# 幂等；兼容 v1/原版两态自动升级；应用前自动备份 .bak-workspacerefresh-<ts>；语法校验失败自动回滚
```
...
**验证**：
1. 经 mcp-proxy 调 `code_use_workspace {workspace: <任意绝对路径>}` → 返回
   `active_workspace` 与 `serena_url`（v2 前未注册路径抛 `project is not registered/authorized`）
2. 重复调用直接命中（已补注册进 ProjectRegistry）
